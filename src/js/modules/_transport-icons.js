...
const tourStepsContainers = Array.prototype.slice.call(
    document.querySelectorAll(".tour-route__steps")
);

tourStepsContainers.forEach(container => {
    console.log("Another tour steps container");
    const tourSteps = Array.prototype.slice.call(
        container.querySelectorAll(".tour-route__step")
    );
    ...
    function setupIcons() {
        ...
        let offset = 0;
        let circleWidth = container.querySelector("figure").offsetWidth;
    ...
    const debouncedSetupIcons = _.debounce(setupIcons, 300);


    setupIcons()

    window.addEventListener('resize', debouncedSetupIcons);
});
